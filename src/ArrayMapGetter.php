...
class ArrayMapGetter extends AbstractArrayOperator
{
     ...
     * @param bool $assoc
     * result will be assoc array with $searchedKeysArray as keys
     * @param unknown $defaultValue
     ...
     *            ('keys' if $mapArray consists of keys, 'values' if $mapArray consists of values, 'pairs' if $mapArray is assoc)
     * @param string $excludeMappers
     *            (if true $mapArray excluded from result)
    ...
    public static function getAssocPairsByMapping($haystack, array $mapArray, $mapType, $excludeMappers = true, $defaultValue = false)
    {
        ...
        if ($excludeMappers && $result != $arrayMapGetter->default) {
            switch ($mapType) {
                case 'keys':
     ...
     * @return array|default value
     */
    protected static function getAssocPairsByMappingKeysRecursion(ArrayOperatorInterface $arrayMapGetter, $haystack, $keysArray)
    {
        $bottom = true;
     ...
     * @return array|default value
     */
    protected static function getAssocPairsByMappingValuesRecursion(ArrayOperatorInterface $arrayMapGetter, $haystack, $valuesArray)
    {
        $bottom = false;
        $temp = $valuesArray;
     ...
     * @return array|default value
     */
    protected static function getAssocPairsByMappingPairsRecursion(ArrayOperatorInterface $arrayMapGetter, $haystack, $pairsArray)
    {
        $bottom = false;
        $temp = $pairsArray;
        foreach ($temp as $tempKey => $tempValue) {
            if (isset($haystack[$tempKey]) && in_array($tempValue, $haystack)) {
                unset($temp[$tempKey]);
            }
        }
        if (empty($temp)) {
